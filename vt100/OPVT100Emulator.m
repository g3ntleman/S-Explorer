...
- (void) puts: (char*) f {
    ...
    strncpy(cbuf,f, 1023);
    strcat(cbuf,"\n");
    [self processedTextOut: cbuf length: strlen(cbuf)];
}
...
- (int) processBracket: (int) Start {
    ...
    switch (cBuffer[End])
    {
            /*         Cursor Up:      Esc [ Pn A */
        case 'A':
            [_delegate setOffsetCursorRow: numargs ? -args[0] : -1 column: 0];
            End += 1;
            break;
        ...
        case 'C':
            [_delegate setOffsetCursorRow: 0 column: numargs ? args[0] : 1];
            End += 1;
            break;
        ...
        case 'f':
            if (numargs == 0)
                [_delegate setAbsoluteCursorRow: 1 column: 1];
            else if (numargs == 1)
                [_delegate setAbsoluteCursorRow: args[0] > 0 ? args[0] : 1 column: 1];
            else if (numargs == 2)
                [_delegate setAbsoluteCursorRow: args[0] > 0 ? args[0] : 1 column: args[1] > 0 ? args[1] : 1];
        ...
        case 'J':
            if (numargs == 0)
                [_delegate eraseTextFromRow: CUR_ROW
                                  andColumn: CUR_COL
                                      toRow: BOTTOM_EDGE
                                  andColumn: RIGHT_EDGE];
            else if (args[0] == 1)
                [_delegate eraseTextFromRow: TOP_EDGE
                                  andColumn: LEFT_EDGE
                                      toRow: CUR_ROW
                                  andColumn: CUR_COL];
            else
        ...
        case 'K':
             ...
             */
        case 'm':
            [_delegate getTextForegroundAttributes: &iForeground backgroundAttributes: &iBackground];
            ...
            if (numargs < 1) {
                ...
                [_delegate setTextForegroundAttributes: iForeground backgroundAtributes: iBackground];
                End += 1;
                break;
            }
        ...
        case 'M':
            do {
                [_delegate deleteTextFromRow: CUR_ROW
                                   andColumn: LEFT_EDGE
                                       toRow: CUR_ROW
                                   andColumn: RIGHT_EDGE];
            } while (--args[0] > 0);
        ...
        case 'r':
            if (numargs == 0)
                [_delegate setScrollingFromRow: TOP_EDGE toRow: BOTTOM_EDGE];
            else if (numargs == 2)
                [_delegate setScrollingFromRow: args[0] toRow: args[1]];
            End += 1;
            break;
        ...
        case 'g':
            if (numargs == 0) {
                [_delegate clearTab: CUR_COL];
            } else {
                if ((numargs == 1) && (args[0] == 3)) {
                    [_delegate clearTab: ALL_TABS];
                }
            }
...
- (int) processEscape: (int) Start {
    ...
    switch (cBuffer[End])
    {
        ...
        case 'B':
            ...
            /*         Cursor right            Esc C */
        case 'C':
            [_delegate setOffsetCursorRow: 0 column: 1];
            End += 1;
            break;
        ...
        case 'E':
            [_delegate writeRawText: "\n" length: 1];
            End += 1;
            break;
            
            /*          Invoke the Graphics character set  Esc F */
        case 'F':
            [_delegate getTextForegroundAttributes: &fore backgroundAttributes: &back];
            if (! (fore & SC_GRAPHICS)) {
                fore &= ~(SC_ASCII|SC_G0|SC_G1);
                fore |= SC_GRAPHICS;
                [_delegate setTextForegroundAttributes: fore backgroundAtributes: back];
            }
            End += 1;
            break;
        ...
        case 'H':
            [_delegate setAbsoluteCursorRow: TOP_EDGE column: LEFT_EDGE];
            End += 1;
            break;
        ...
        case 'Z':
            [_delegate transmitText: ANSWERBACK_MESSAGE length: strlen(ANSWERBACK_MESSAGE)];
            End += 1;
            break;
        ...
        case '7':
            [_delegate saveCursor];
            End += 1;
            break;
            
            /*         Restore cursor position & attributes:  Esc 8 */
        case '8':
            [_delegate restoreCursor];
            End += 1;
            break;
        ...
        default: {
            ...
            [_delegate writeRawText: cbuf+Start length: 6];
            End += 1;
        }
    }
...
- (int) processControl: (int) Start {
    ...
    switch (cBuffer[Start]) {
        ...
        default: {
            ...
            [_delegate writeRawText: buf length: 2];
            End += 1;
        }
            break;
    }
...
- (int) processBuffer {
    ...
    while (Start < BufLen) {
        ...
        if (End > Start) {
            [_delegate writeRawText: cBuffer+Start length: End-Start];
        }
...
- (int) processedTextOut: (const char*) cbuf length: (size_t) count {
    ...
    if ((count + BufLen) > MAXVTBUFLEN) {
        [_delegate writeRawText: "ERROR: VT-100 internal buffer overflow!" length: 39];
        return(-1);
    }
