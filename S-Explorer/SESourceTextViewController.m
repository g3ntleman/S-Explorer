...
- (BOOL) textView: (NSTextView*) textView shouldChangeTextInRange: (NSRange) affectedCharRange replacementString: (NSString*) replacementString {
    ...
    [self.textView.textStorage unmarkPar];
...
- (NSRange) textView: (NSTextView*) textView willChangeSelectionFromCharacterRange: (NSRange) oldRange toCharacterRange: (NSRange) newRange {
    ...
    if (newRange.length == 1) {
        ...
        if (isPar(theChar)) {
            ...
            BOOL match = [self.textView.textStorage expandRange: &parRange toParMatchingPar: theChar];
        ...
        [self.textView.textStorage markParCorrespondingToParAtIndex: MIN(oldRange.location, newRange.location)];
    }
